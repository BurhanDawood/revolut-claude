...
>
> **Owner:** Bryan. **Last updated:** 2026-06-22. (#49 MSS structure tracker A+B; #50 Build 2 abnormal-move detector A+B; #40 catalyst calendar + in-chat research model — weekly paid sweep removed; daily brief-reminder cron; #14/#19/#20/#23 dashboard+dedup cluster; earlier: #93 auto-exec trailing stop, #24 per-coin opt-in, #125 Away Mode 2c, #130 trough-rebuy, #143 standalone trough-buy.)
...
- **MCP tools (16 active):** `get_portfolio_summary`, `get_portfolio_data`, `get_trading_data`, `get_context`, `manage_alerts`, `manage_trading`, `set_entry_price`, `execute_kraken_trade`, `set_auto_trade_rule`, `get_auto_rules`, `manage_auto_rules`, `get_prices`, `get_tranches`, `research_asset`, `set_pump_armed_rule`, `resolve_pending_trades`. The tool list/count is seeded into `system_capabilities` config at boot; `project_description` roadmap updated 2026-06-20 (#2).
...
- **Auto-exec / pump-loop:** #95 built, BOBA live cycles running. #93 auto-exec trailing stop done. #24 per-coin opt-in done. #45 per-coin sell-floor done. #125 Away Mode done (Phase 2c proven live). #130 trough-rebuy done. #143 standalone trough-buy done. Remaining: upgrade #130 rebuy to trailing trough tracker (needs PC/Cowork, spec in `rebound_tracker_spec` pref); #46 availability-gated; #33 emergency liquidation; #34 pump-capture mode.
