...
// Tries multiple pair formats Kraken uses (e.g. GHIBLIUSD, ZKUSD, TAOUSD)
async function getKrakenPriceForSymbol(symbol) {
  const coinBase = symbol.replace('-USD', '');
  const krakenBase = coinBase === 'BTC' ? 'XBT' : coinBase;
  const pairs = [`${krakenBase}USD`, `X${krakenBase}ZUSD`, `${krakenBase}ZUSD`];

  for (const pair of pairs) {
    try {
      const res = await fetch(`${KRAKEN_API_URL}/0/public/Ticker?pair=${pair}`);
      const data = await res.json();
      if (data.result) {
        const entry = Object.values(data.result)[0];
        const price = parseFloat(entry?.c?.[0]);
        if (price > 0) {
          console.log(`[kraken] ${symbol} price: $${price} (pair: ${pair})`);
          return price;
        }
      }
    } catch (_) { /* try next format */ }
  }
  console.log(`[kraken] Could not fetch price for ${symbol}`);
  return null;
}
