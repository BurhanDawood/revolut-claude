...
async function getDndMode() {
  try {
    const [r] = await db.execute("SELECT config_value FROM system_config WHERE config_key = 'dnd_mode'");
    return r.length ? JSON.parse(r[0].config_value) : { enabled: false, coins: [] };
  } catch (e) { return { enabled: false, coins: [] }; }
}
async function setDndMode(obj) {
  await db.execute("INSERT INTO system_config (config_key,config_value) VALUES ('dnd_mode',?) ON DUPLICATE KEY UPDATE config_value=VALUES(config_value)", [JSON.stringify(obj)]);
}
async function isDndCoin(coinBase) {
  const d = await getDndMode();
  return d.enabled && Array.isArray(d.coins) && d.coins.includes(coinBase.toUpperCase());
}
...
// ── #8: unified realised-P&L (USD) writer — called from every sell path ──────
// Writes trading_journal.realised_pnl_usd = (sale - avg entry) * qty in DOLLARS so
// per-asset lifetime realised totals span ALL sell sources (not just auto_detected).
// Cost basis = current avg entry (entry_prices), keyed by -USD symbol (base fallback).
// Additive only: never alters execution or the existing outcome_pnl write.
async function recordRealisedPnl(journalId, symbolUsd, salePrice, saleQty) {
  try {
    if (!journalId || !(parseFloat(salePrice) > 0) || !(parseFloat(saleQty) > 0)) return null;
    const base = String(symbolUsd).replace('-USD', '');
    let entry = entryPrices.get(symbolUsd);
    if (entry == null) entry = entryPrices.get(base);
    if (entry == null) {
      const [er] = await db.execute('SELECT entry_price FROM entry_prices WHERE symbol = ? OR symbol = ? LIMIT 1', [symbolUsd, base]);
      if (er && er.length) entry = parseFloat(er[0].entry_price);
    }
    if (entry == null || !(parseFloat(entry) > 0)) {
      console.log('[pnl-usd] no cost basis for ' + symbolUsd + ' — realised_pnl_usd left NULL on journal ' + journalId);
      return null;
    }
    const realised = (parseFloat(salePrice) - parseFloat(entry)) * parseFloat(saleQty);
    await db.execute('UPDATE trading_journal SET realised_pnl_usd = ? WHERE id = ?', [realised.toFixed(8), journalId]);
    console.log('[pnl-usd] ' + base + ' realised $' + realised.toFixed(2) + ' (entry $' + entry + ', sale $' + parseFloat(salePrice) + ', qty ' + parseFloat(saleQty) + ') -> journal ' + journalId);
    return realised;
  } catch (e) {
    console.error('[pnl-usd] recordRealisedPnl error:', e.message);
    return null;
  }
}
    ...
    if (action === 'sell') {
      ...
      try {
        ...
        if (sellEntryPrice && salePrice && saleQty) {
          ...
          // #8: also persist unambiguous USD realised P&L (shared-column source of truth)
          await db.execute('UPDATE trading_journal SET realised_pnl_usd = ? WHERE id = ?', [realisedPnl.toFixed(8), journalId]).catch(() => {});
        ...
        if (entryPrice && newBuyPrice < entryPrice * 0.95) {
          ...
          try {
            ...
            if (pumpRows.length) {
              ...
              console.log('[trough] ' + coinBase + ' pump-loop sell: trough tracker armed (proceeds $' + availableForBuyback.toFixed(2) + ')');
              usedTroughTracker = true;
              // E5: armReboundTracker sends its own Telegram, suppress generic cascadeMsg
            }
    ...
    async ({ action, symbol, trade_action, price, quantity, reasoning, emotion, followed_recommendation, expires_hours, key, value, amount, away_buy_usd, away_sell_pct, capital_type, note, enabled, sweep_pct, min_trade_value_usd, excluded_symbols, max_sell_pct, max_buy_usd, allowed_triggers, require_confidence, cooldown_minutes, hodl_symbols: hodlSymbolsParam, title, detail, category, status: devStatus, source: devSource, related_symbol: relSymbol, dev_log_id, active_workstream, progress, open_threads, next_action, recent_decision, recent_decisions, cs_status, cs_role, cs_theme, cs_strategy_md, pm_decision, pm_principle_tag, pm_conviction, pm_captured_by, pm_supersedes_id, dev_decision, dev_principle_tag, dev_cross_thread, dev_alternatives, dev_related_log, dev_supersedes_id, journal_id, tax_lot_id, away_action, away_coins, sell_floors, per_coin_enabled, catalyst_id, catalyst, catalyst_date, catalyst_type, expected_impact, priced_in_risk, catalyst_confidence, catalyst_source, catalyst_status, abn_alert_floor_pct, abn_broad_floor_pct, abn_broad_threshold, abn_cooldown_min, dnd_action, dnd_coins, dnd_arm_pct, dnd_trail_pct, dnd_sell_pct, dnd_retrace_pct, dnd_bounce_pct }) => {
      ...
      if (action === 'log_journal') {
        ...
        if (awayAction === 'set_eligible') {
          ...
          const buyAmt = away_buy_usd ?? amount; // #145 dedicated param preferred, falls back to amount
          if (!buyCoin || buyAmt == null || buyAmt <= 0) {
            reply = `set_away_buy needs away_coins=[COIN] and away_buy_usd>0. Current: ${Object.keys(am.away_buy_usd).length ? Object.entries(am.away_buy_usd).map(([k,v])=>k+' $'+v).join(', ') : '(none set)'}`;
          } else {
        ...
        if (sub === 'set_params') {
          ...
          if (dnd_retrace_pct != null) p.retrace_pct = Number(dnd_retrace_pct);
          if (dnd_bounce_pct  != null) p.bounce_pct  = Number(dnd_bounce_pct);
        ...
        if (sub === 'activate') {
          ...
          const D = { arm: dnd_arm_pct ?? dp.arm_pump_pct ?? 30, trail: dnd_trail_pct ?? dp.trail_pct ?? 8, sell: dnd_sell_pct ?? dp.sell_pct ?? 100, retrace: dnd_retrace_pct ?? dp.retrace_pct ?? 50, bounce: dnd_bounce_pct ?? dp.bounce_pct ?? 8 };
          for (const coin of coins) {
            const sym = `${coin}-USD`;
            const [ex] = await db.execute('SELECT symbol FROM pump_armed_rules WHERE symbol=? AND active=1', [sym]);
            if (!ex.length) {
              await db.execute('INSERT INTO pump_armed_rules (symbol,arm_pump_pct,arm_window_min,trail_pct,retrace_pct,bounce_pct,sell_pct,loop_enabled,active) VALUES (?,?,60,?,?,?,?,1,1)', [sym,D.arm,D.trail,D.retrace,D.bounce,D.sell]);
            } else {
              await db.execute('UPDATE pump_armed_rules SET loop_enabled=1,arm_pump_pct=?,trail_pct=?,sell_pct=?,retrace_pct=?,bounce_pct=? WHERE symbol=? AND active=1', [D.arm,D.trail,D.sell,D.retrace,D.bounce,sym]);
            }
          }
...
app.post('/telegram-webhook', async (req, res) => {
  try {
    ...
    if (dndMatch && commandText !== 'dnd off') {
      ...
      const DND = { arm_pump_pct: 30, trail_pct: 8, retrace_pct: 50, bounce_pct: 8, sell_pct: 100 };
      for (const coin of coins) {
        const sym = `${coin}-USD`;
        const [ex] = await db.execute('SELECT symbol FROM pump_armed_rules WHERE symbol = ? AND active = 1', [sym]);
        if (!ex.length) {
          await db.execute(
            'INSERT INTO pump_armed_rules (symbol,arm_pump_pct,arm_window_min,trail_pct,retrace_pct,bounce_pct,sell_pct,loop_enabled,active) VALUES (?,?,60,?,?,?,?,1,1)',
            [sym, DND.arm_pump_pct, DND.trail_pct, DND.retrace_pct, DND.bounce_pct, DND.sell_pct]
          );
        } else {
          await db.execute('UPDATE pump_armed_rules SET loop_enabled=1 WHERE symbol=? AND active=1', [sym]);
        }
      }
    ...
    // Command: dnd off (#32)
    if (commandText === 'dnd off') {
      const d = await getDndMode();
      await setDndMode({ enabled: false, coins: d.coins || [] });
      await sendReply('\u2600\ufe0f DND OFF. Pump loop returns to standard behaviour.');
      return res.status(200).json({ ok: true });
    }
    if (commandText === 'status') {
      ...
      const dnd32 = await getDndMode().catch(() => ({ enabled: false, coins: [] }));
      const dndLine = dnd32.enabled ? `\ud83c\udf19 DND: ON (${(dnd32.coins||[]).join(', ')})\n` : '';
      await sendReply(dndLine + statusMsg);
      return res.status(200).json({ ok: true });
    }
