...
>
> **Owner:** Bryan. **Last updated:** 2026-06-20. (#8 lifetime ledger; #94/#99 fast-scan arc; #120 pmRec; #87 trailing-stop plan-aware; #31 exit-only gate; #26 sweep profit basis; #122 rebalance journal; #5 dedup fail-safe; #2 tool count 16; #63 fiat withdrawal; #60 baseline24h window.)
...
**Fiat withdrawal auto-log (#63, 2026-06-20).** Parallel detector: when USD (not USDT) decreases >$5 with no USDT-conversion offset and no recent crypto buy, the drop is auto-logged as `source='fiat_withdrawal'` + capital decremented + Telegram notify. Reversible via `skip payment [amount]` (handler now covers both `revolut_card` and `fiat_withdrawal`). A 30-min dupe-guard on `withdrew X` prevents double-decrement if both the auto-detect and a manual reply fire for the same amount.
...
- **#94 / #99** — fast-scan quality arc (2026-06-20). #94: `runFastScan()` discarded `updateTrailingStop()` return value — breaches swallowed, never routed to `handleTrailingStopAlert`. 1-line fix. #99: replaced hardcoded `FAST_SCAN_SYMBOLS` trailing-stop loop with dynamic `trailingStops.keys()` + single batched `/tickers` call per cycle. All armed trailing stops now scanned at 30s regardless of coin. *Both resolved 2026-06-20.*
...
- **#87** — trailing-stop alert plan-aware (2026-06-20). `analyseTrailingStopAlert()` was the last plan-blind path. Fix mirrors A3: `getCoinContext` → role check → `coin_strategy` read → dynamic system prompt with `## SAVED PLAN`. HODL/manual-only coins now get `[HOLD / RESET STOP]` menu only. *Resolved 2026-06-20.*
- **#31 remainder** — exit-only role gate all paths (2026-06-20). `getCoinContext` DB fallback for dead_bag/legacy_exit; gate propagated across `getQuickAiRecommendation`, A2 swing-detect, A3 fixed-target, `analyseTrailingStopAlert`, `buildPlanAwareSwingSignal`. XPL + PYTH now get HOLD/exit messaging on all dip alerts. *Resolved 2026-06-20.*
- **#26** — USDT sweep profit basis (2026-06-20). Was computing `proceeds × sweep_pct`. Fix: optional `realisedPnlUsd` param to `sweepToUSDT()`; all 3 callers pass `(salePrice − entry) × qty`; losing trades skip sweep entirely. *Resolved 2026-06-20.*
- **#122** — rebalance journal action corrected (2026-06-20). `logRebalancePair()` set both legs to `action='transfer'` — invisible to `gradeTradeOutcomes` (queries `action IN ('buy','sell','add','reduce')`). Fix: sell leg → `'sell'`, buy leg → `'buy'`. *Resolved 2026-06-20.*
- **#5** — `autoLogTrade` dedup fail-safe (2026-06-20). B2a `pending_orders` suppression check and CHECK 3 quantity-dedup both fell open on exception. Both now `return` on error. Root cause of AVAX double-log confirmed as CHECK 3 throwing on a DB blip. *Resolved 2026-06-20.*
- **#63** — fiat withdrawal auto-log (2026-06-20). When USD decreases >$5 with no USDT-conversion offset and no recent buy: auto-log `source='fiat_withdrawal'`, decrement capital, notify Telegram. `skip payment` handler broadened to cover both sources. `withdrew X` command has 30-min dupe-guard to prevent double-deduct. *Resolved 2026-06-20.*
- **#60 / #123** — baseline24h window widened (2026-06-20). `price_history` only has midnight rows. At 9 AM boot the nearest row was outside the old 22–26h window → all coins showed 0% move. Fix: widen to 12–36h ago; `MIN(ABS(TIMESTAMPDIFF))` self-join reliably finds the nearest midnight row at any boot time. *Resolved 2026-06-20.*
...
- **Tier 2 — core quality / intelligence layer:** #72 done; #36 plan-aware alerts done (A1/A2/A3 + #87 trailing-stop path, 2026-06-20); #90 parser done 2026-06-19. Remaining: #50 Build 2 ATR detector (earliest July — needs price_intraday accrual), #49 MSS tracker, #40 catalyst calendar. Umbrella #68; sources #61, #69.
- **Tier 2.5 — learning loop:** #48 outcome loop (Opus, own session); #122 rebalance journal fix (2026-06-20) ensures rotation legs are now graded when #48 ships. → #52 shadow tracker → #54 emotion×outcome → #51 checklist gate → #53 concentration dashboard.
- **Auto-exec / pump-loop:** #95 built end-to-end (OFF). Follow-ups: MCP setter for `loop_enabled`/`max_cycles`; exact loop P&L; #45 per-coin sell-floor; #24/#46/#33/#34. Single cycle never run live.
- **Known bugs to watch:** #76 (macro alert guardrails partial — Haiku can still produce stale narratives), #74 (context-blind ladder rec), #44 (multi-trade approval clobber — Opus, own session). #75 dashboard alerts render resolved 2026-06-19. #99 dynamic fast-scan resolved 2026-06-20.
